...
| ~~**Ảnh được dựng trên VPS**~~ **ĐÃ ĐẨY LÊN GHCR** | CI dựng ảnh và đẩy lên `ghcr.io`, máy chủ **kéo về** thay vì tự dựng — thứ CI kiểm và thứ đang phục vụ nay là MỘT nhị phân. Ảnh `api` dùng chung hai môi trường (Java đọc cấu hình lúc chạy); ảnh `frontend` **tách theo môi trường** vì Vite nướng URL vào bundle lúc build. Xem §6.1 của `devops/PIPELINE_AND_DEPLOY.md` |
| ~~**Không có tự động tạo release/tag**~~ **ĐÃ CÓ** | `cd.yml` tạo tag `vNĂM.THÁNG.NGÀY.<số chạy>` + GitHub release **sau khi** production deploy xong và `/api/health` đã trả lời. Deploy hỏng rồi tự lùi thì KHÔNG sinh tag — không có gì mới đang chạy để mà ghi. Chạy lần đầu 09/09/2026: `v2026.09.09.92`. *(Con số "3 tag" ở bản trước là sai: đó là tag LOCAL của lịch sử nhóm, chưa từng đẩy lên fork này — remote thật sự có 0 tag.)* |
